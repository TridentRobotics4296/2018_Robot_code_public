...
public class TurnByNDegreesNavXStep extends AutoStep
{
   ...
   private static final double TURN_EXPONENT = 1.5; //Adjust for turning speed curve. Higher = faster theoretical turn
   private static final double MAX_SPEED = 0.4;
   
   public TurnByNDegreesNavXStep(int p_deltaHeading, double p_minRotation) 
   {
   ...
   public void initialize()
   {
      ...
      m_currentHeading = getCompassHeading(m_drive.getYaw());
   ...
   {      
      ...
      double speed = calculateRotationSpeed(m_currentHeading, m_target, TOLERANCE);
      m_drive.setHeading(speed);
      ...
      SmartDashboard.putNumber("Turn rotation speed", speed);
   }
   ...
   private double calculateRotationSpeed(int p_current, int p_target, int p_tolerance)
   {
      ...
      rotationSpeed = (((double)distanceToTarget * (MAX_SPEED - m_minRotation))/ 180) + m_minRotation;
   ...
   {
      return "Turn to heading navX step";
   }
