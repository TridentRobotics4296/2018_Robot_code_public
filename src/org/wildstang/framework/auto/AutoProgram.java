...
public abstract class AutoProgram
{
   ...
   public void update()
   {
      ...
      AutoStep step = (AutoStep) programSteps.get(currentStep); // Prevent errors caused by mistyping.
   ...
   protected void loadStopPosition()
   {
      ...
      if (forceStopAtStep != 0)
      {
         ...
         if ((forceStop <= programSteps.size()) && (forceStop > 0))
         {
            ...
            s_log.logp(Level.ALL, "Auton", "Force Stop", "Program is forced to stop at Step "
                  + forceStop);
         }
         else
         {
            s_log.logp(Level.SEVERE, "Auton", "Force Stop", "Force stop value is outside of bounds. (0 to "
                  + (programSteps.size() - 1));
         }
      }
   }
